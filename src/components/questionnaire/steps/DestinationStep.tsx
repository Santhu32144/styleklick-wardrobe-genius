...
const DestinationStep: React.FC<DestinationStepProps> = ({ 
  ...
  const handleDestinationSelect = (destination: Destination) => {
    updateFormData({ destination, destinationType: destination });
  };
        ...
        {destinationOptions.map((option) => (
          <Card 
            ...
            className={`cursor-pointer transition-all duration-200 hover:shadow-md ${
              (formData.destination === option.value || formData.destinationType === option.value) ? 'ring-2 ring-styleklick-purple border-styleklick-purple' : 'border-gray-200'
            }`}
          ...
          >
            <CardContent className="flex flex-col items-center p-6">
              <div className={`p-4 rounded-full mb-4 ${
                (formData.destination === option.value || formData.destinationType === option.value) ? 'bg-styleklick-purple text-white' : 'bg-gray-100 text-gray-600'
              }`}>
                {option.icon}
              </div>
              <h4 className="font-medium text-lg mb-2">{option.label}</h4>
              <p className="text-sm text-gray-500 text-center">{option.description}</p>
              {(formData.destination === option.value || formData.destinationType === option.value) && (
